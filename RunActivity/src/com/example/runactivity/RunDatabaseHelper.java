...
public class RunDatabaseHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		+ " altitude real, provider varchar(100), run_id integer "
		+ "references run(_id))");
	}
	...
	public RunCursor queryRuns() {
		// Equivalent to "select * from run order start_date asc"
	...
	public LocationCursor queryLastLocationForRun(long id) {
	...
	public LocationCursor queryLocationsForRun(long runId) {
		Cursor wrapped = getReadableDatabase().query(TABLE_LOCATION, 
				...
				null, // group by 
				null, // having
				COLUMN_LOCATION_TIMESTAMP + " asc"); // order chronologically
		return new LocationCursor(wrapped);
	}
	...
	public static class LocationCursor extends CursorWrapper {
		...
		public Location getLocation() {
			if (isBeforeFirst() || isAfterLast()) return null;
			// First get the provider so you can use the constructor
